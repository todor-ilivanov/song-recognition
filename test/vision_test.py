...
def mock_vision_response(text):
    text_annotations = TextAnnotations(text)
    response = AnnotateImageResponse([text_annotations])
    return response

def test_extract_track_name_success():
    vision_api.client.text_detection = MagicMock(return_value=mock_vision_response("It's Random Song by Random Band"))
...
def test_extract_track_name_error():
...
def test_extract_track_name_invalid_image():
    vision_api.client.text_detection = MagicMock(return_value=mock_vision_response("Found some text but not a song name."))
    response = vision_api.request(TEST_IMAGE_PATH)
    track_name = vision_api.extract_track_name(response)
    assert track_name is None

def test_extract_track_name_malformed_response():    
